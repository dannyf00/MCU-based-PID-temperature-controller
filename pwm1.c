...
void pwm1_init(uint8_t prescaler) {
	...
	TIMSK =		//(0<<TICIE1) |				//input capture isr: disabled
				...
				0x00;
	//configure the pins
#if defined(PWM1A_PIN)
	IO_OUT(PWM1A_DDR, PWM1A_PIN);				//oc1a as output
#endif
#if defined(_PWM1A_PIN)
	IO_OUT(PWM1A_DDR, _PWM1A_PIN);				//oc1a as output
#endif
...
#if defined(PWM1B_PIN)
	IO_OUT(PWM1B_DDR, PWM1B_PIN);				//oc1b as output
#endif
#if defined(_PWM1B_PIN)
	IO_OUT(PWM1B_DDR, _PWM1B_PIN);				//oc1b as output
#endif
...
#endif
				0x00;
	TCCR1 =		(TCCR1 &~TMR1_PS_MASK) | (prescaler & TMR1_PS_MASK);	//prescaler, per the header file
